...
2. The program begins by asking the user for the operating system path. Expected responses: mac, bash (for Linux), nt (for Windows), etc.
...
5. The program counts the number of terms in the dictionary, omitting linked pages that are not in the A-Z index.
6. The program asks the user if the user would like to update a definition, prompting the user with a yes/no question, then prompting the user for the term and definition and repeating this step until the user indicates that they are done updating definitions.
...
7. The user is asked for a filename to save the file as.
8. The program writes the output to the designated text file and saves this file.
9. This output contains three columns: Term, Definition and URL. An example file is one created for Mac, which should contain 299 terms, definitions and URLs.
